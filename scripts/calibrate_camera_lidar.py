...
def extract_points_3D(velodyne, now):
    ...
    if OUSTER_LIDAR: points = points.reshape(-1, 4)[:, :3]
    ...
    ax.set_title('Select 3D LiDAR Points - %d' % now.secs, color='black')
    ax.set_axis_off()
...
def project_point_cloud(velodyne, img_msg, image_pub):
    ...
    try:
        transform = TF_BUFFER.lookup_transform('world', 'os_sensor', rospy.Time())	
    ...
    inrange = np.where((points2D[:, 0] >= 0) &	# 사진 범위 내에 있는 points만 통과
                       (points2D[:, 1] >= 0) &
